...
#defining the convert
def convert():
    celTemp = celTempVar.get()
    fahTemp = fahTempVar.get()


#if and elif statements for celcuis and fahrenheit
...
#defining the activate function
def act_cel():
    celEntry.configure(state=NORMAL)


#button for activate celcuis
B1=Button(dinges, text="activate celcuis" , command=act_cel)
B1.grid(row=12, column=10, pady=10)

def act_fah():
    fahEntry.configure(state=NORMAL)

#button for activate fahrenheit
